...
InputFileW::InputFileW(PayloadId payload_id, std::int64_t size)
    : impl_(std::unique_ptr<nearby::InputFile, nearby::InputFileDeleter>(
          new nearby::InputFile(payload_id, size))) {}
InputFileW::InputFileW(std::string file_path, std::int64_t size)
    : impl_(std::unique_ptr<nearby::InputFile, nearby::InputFileDeleter>(
          new nearby::InputFile(file_path, size))) {}
InputFileW::InputFileW(InputFileW&& other) noexcept
    : impl_(std::move(other.impl_)) {}
InputFileW& InputFileW::operator=(InputFileW&& other) noexcept {
  impl_ = std::move(other.impl_);
  return *this;
}
...
std::int64_t InputFileW::GetTotalSize() const { return impl_->GetTotalSize(); }
